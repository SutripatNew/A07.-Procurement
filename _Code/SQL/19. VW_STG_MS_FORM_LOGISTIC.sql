...
/****** Object:  View [dbo].[VW_STG_MS_FORM_LOGISTIC]    Script Date: 20/5/2568 9:26:22 ******/
SET ANSI_NULLS ON
GO
...
--			WHERE QUESTION_NAME IN ('ปีที่ประเมิน','Submission Time','Responder Email','Level1','Level2','Level3','ผู้รับการประเมิน ALL','ข้อเสนอแนะ เพื่อปรับปรุงการบริการของ Vendor','โรงงาน','Above1MB','รอบการประเมิน','BelowThreshold', 'Pur_Grp')
--		) AS F
--		PIVOT  
--		(  
--		MIN(RESPONSE) FOR QUESTION_NAME IN ([ปีที่ประเมิน], [Submission Time],[Responder Email],[Level1],[Level2],[Level3],[ผู้รับการประเมิน ALL],[ข้อเสนอแนะ เพื่อปรับปรุงการบริการของ Vendor],[โรงงาน],[Above1MB],[รอบการประเมิน],[BelowThreshold], [Pur_Grp])
--		) AS P
--	) A 
--	LEFT JOIN (
--		SELECT SHAREPOINT_LIST,ITEM_ID,QUESTION_NAME,RESPONSE FROM STG_MS_FORM_LOGISTIC
--		WHERE QUESTION_NAME NOT IN ('ปีที่ประเมิน','Submission Time','Responder Email','Level1','Level2','Level3','ผู้รับการประเมิน ALL','ข้อเสนอแนะ เพื่อปรับปรุงการบริการของ Vendor','โรงงาน','Above1MB','รอบการประเมิน','BelowThreshold', 'Pur_Grp')
--	) B
...
(
	...
	ON A.formyear = D.[YEAR] AND LEFT(C.question_name,4) = D.QUESTION_CODE AND  N'Logistic' =  D.LEVEL1
) M
