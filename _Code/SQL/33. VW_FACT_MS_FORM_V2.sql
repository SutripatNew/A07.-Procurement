...
/****** Object:  View [dbo].[VW_FACT_MS_FORM_V2]    Script Date: 23/5/2568 14:38:22 ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW [dbo].[VW_FACT_MS_FORM_V2] AS 


/*
The newly created process will not include VW_FORM_07_SCORE_FIX.
Specifically, the flow will be:
VW_FORM_06_SCORE_V2 >> FACT_MS_FORM_SCORE_V2 >> >> VW_FACT_MS_FORM_V2
This is to ensure that VW_FACT_MS_FORM_V2 has the same structure as VW_FACT_MS_FORM.
As a result, there might be some extra columns that are not being used, as listed below:
W_FACT(Weight_Report to Score_PCM_Format)
*/

WITH W_FACT AS (

SELECT 
F.*, Weight_Source	AS Weight_Report, Score AS Score_Report, 
	[Weight_Source] AS Weight_UniqueID,
	Score			AS Score_UniqueID,
	Weight_Source	AS Weight_Calc,
	Score			AS Score_Calc,
	FORMAT(Raw_Score, '0.00')  + ' x ' + FORMAT(Weight_Source, '0.00') AS Score_Calc_Format,
	Weight_Source	AS Weight_RM,
	Score			AS Score_RM,
	FORMAT(Raw_Score, '0.00')  + ' x ' + FORMAT(Weight_Source, '0.00') AS Score_RM_Format,
	Weight_Source	AS Weight_PCM,
	Score			AS Score_PCM,
	FORMAT(Raw_Score, '0.00')  + ' x ' + FORMAT(Weight_Source, '0.00') AS Score_PCM_Format

--CASE 
--WHEN Function_Name IN ('RM&PKG', 'Commercial', 'Manufacturing')
--THEN F.Weight_RM
...
--AS Score_Report
FROM FACT_MS_FORM_SCORE_V2 F

)
,
GROUP_BY_Pur_Grp AS (
	SELECT FormYear,Function_Name, Pur_Grp, Vendor_Name, Title, SUM(Score) AS Score, SUM(Weight_Source) AS Weight_Source
	FROM W_FACT
	GROUP BY FormYear,Function_Name, Pur_Grp, Vendor_Name, Title
)
,
C_ScoreTotal1 AS (
	SELECT FormYear,Function_Name, Pur_Grp, Vendor_Name,AVG(Score) AS Score,AVG(Weight_Source) AS Weight_Source
	FROM GROUP_BY_Pur_Grp
	GROUP BY FormYear,Function_Name, Pur_Grp, Vendor_Name
)
,
C_ScoreTotal AS (
	SELECT FormYear,Function_Name, Vendor_Name,SUM(Score) AS Score,SUM(Weight_Source) AS Weight_Source
	FROM C_ScoreTotal1
	GROUP BY FormYear,Function_Name, Vendor_Name
)
...
/* ปรับแก้ Grade ใหม่ */
LEFT JOIN (
	SELECT *, Score/Weight_Source*100 AS SumScoreByVendorLevel
	FROM C_ScoreTotal
) M
ON F.FormYear = M.FormYear
AND F.Function_Name = M.Function_Name
AND F.Vendor_Name = M.Vendor_Name
...
LEFT JOIN (	
    ...
    FROM(
        ...
        SUM(Score) AS RawVendorLevelScore,
        COUNT(DISTINCT UniqueID) AS RawVendorLevelFormCount,
        SUM(Score) / COUNT(DISTINCT UniqueID) AS RawVendorLevelScore_Weighted
        FROM dbo.FACT_MS_FORM_SCORE_V2
 ...
 --           Weight_Source AS Weight, Weight_Source,RoundYear,
 --           SUM(Score) as SumUnique,
 --            COUNT(DISTINCT UniqueID) as CountUnique ,
 --           SUM(Score) / COUNT(DISTINCT UniqueID) AS RawVendorLevelScore_Weighted
 --           FROM dbo.FACT_MS_FORM_SCORE_V2
 --           WHERE Function_Name = 'RM&PKG' 
 --           GROUP BY FormYear,Function_Name, Function_SubType, Vendor_Name, /*Level1, Level2, */ Group_Question, Pur_Grp, Question_Name, Weight_Source,RoundYear
 --       ) A
...
LEFT JOIN (
    ...
    FROM (
        ...
        FROM(
            ...
            Weight_Source AS Weight, Weight_Source,
            SUM(Score) as SumUnique,
             COUNT(DISTINCT UniqueID) as CountUnique ,
            SUM(Score) / COUNT(DISTINCT UniqueID) AS RawVendorLevelScore_Weighted
            FROM dbo.FACT_MS_FORM_SCORE_V2
            WHERE Function_Name = 'Procurement'
            GROUP BY FormYear,Function_Name, Function_SubType,  ResponderEmail, Vendor_Name, /*Level1, Level2, */  Group_Question, Pur_Grp, Question_Name, Weight_Source   
        ) A
